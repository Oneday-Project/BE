...
export class BasicPapersService {
  ...
  async fetchByRanking(token?: string, query?: string, sort?: string, field?: string, category?: string) {
    ...
    let url =
      ...
      `&sort=citationCount:desc&limit=50`;                                   // SS API는 citationCount만 정렬 지원
  ...
  // ══════════════════════════════════════════════════════════════════
  // 4. 특정 논문 수집 (공백으로 구분된 arxivId 복수 처리)
  // ══════════════════════════════════════════════════════════════════
  ...
  async integrate() {
    ...
    // ── 2. SS 데이터를 arxivId 기준 Map으로 변환 (arXiv 데이터와 빠르게 매핑하기 위함) ──
    const ss2Map = new Map(ss2List.map((s) => [s.arxivId, s]));
